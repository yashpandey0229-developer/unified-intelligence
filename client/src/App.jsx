...
function App() {
  ...
  return (
    // Fragment use karna zaroori hai taaki multiple children return ho sakein
    <>
      <SignedOut>
        <LandingPage />
      </SignedOut>
      ...
      <SignedIn>
        <div className="min-h-screen bg-slate-50 pb-20">
          ...
          <main className="max-w-4xl mx-auto p-6">
            <div className="bg-white p-8 rounded-[2rem] shadow-sm border mb-8">
              <textarea 
                className="w-full p-4 bg-slate-50 rounded-2xl mb-4 h-32 border outline-none text-slate-700" 
                value={input} 
                onChange={(e) => setInput(e.target.value)} 
                placeholder="Analyze issue with Gemini 2.5 Flash..."
              />
              <button 
                onClick={handleAnalyze} 
                disabled={loading} 
                className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black tracking-widest uppercase text-xs"
              >
        ...
        {/* Modal SignedIn ke andar rakha hai taaki sirf login users ko dikhe */}
        {selectedCase && (
